...
import { X, CheckCircle, Loader2, AlertCircle, UserPlus } from "lucide-react";
...
const SOSButton = () => {
  ...
  // Store contacts in state — read fresh each time SOS is clicked
  const [contacts, setContacts] = useState<EmergencyContact[]>([]);
  const [emailContacts, setEmailContacts] = useState<EmergencyContact[]>([]);

  const handleSOS = () => {
    // Read contacts FRESH from localStorage (which is synced from Firestore)
  ...
  const fireSOS = async () => {
    ...
    // Send real emails via backend — read contacts fresh again to be safe
  ...
  return (
    <>
      <button
        onClick={handleSOS}
        className="fixed bottom-6 right-6 z-50 w-16 h-16 rounded-full bg-primary text-primary-foreground font-heading font-bold text-sm flex items-center justify-center shadow-lg hover:scale-110 transition-transform group"
      >
            ...
            >
              ...
              {stage === "no-contacts" && (
                <div className="space-y-4 mt-6">
                  ...
                  <div className="flex gap-3 justify-center mt-4">
                    <button
                      onClick={() => { handleCancel(); navigate("/profile"); }}
              ...
              {stage === "countdown" && (
                <div className="space-y-4 mt-6">
                  ...
                  <p className="text-xs text-muted-foreground">Sending to {emailContacts.length} contact{emailContacts.length !== 1 ? "s" : ""}</p>
                  <button
                    onClick={handleCancel}
                  ...
                  <div className="space-y-2 mt-4">
                    {contacts.map((c) => (
                      <div key={c.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-muted">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-background" style={{ backgroundColor: c.color }}>
